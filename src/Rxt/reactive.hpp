...
#define RXT_observe_capref(val_, ...) (::Rxt::_det::proxy_adder{(val_)}) = [&](__VA_ARGS__)
#define RXT_observe_capval(val_, ...) (::Rxt::_det::proxy_adder{(val_)}) = [=](__VA_ARGS__)
#define RXT_observe RXT_observe_capref
// #define RXT_observe(val_, ...) RXT_set_lambda(::Rxt::_det::proxy_adder{(val_)}, __VA_ARGS__)
...
struct adapt_reactive_template
    : Crt<adapt_reactive_template<Crt, Hook, T...>, T...>
{
    using value_type = Crt<adapt_reactive_template<Crt, Hook, T...>, T...>;
...
struct hook_router
{
    ...
    hook_router()
    {
        ...
        RXT_observe(_dispatch.on_enable) { on_update(); };
        RXT_observe(_dispatch.on_disable) { on_update(); };
    }
    ...
    {
        hook_type hook;
        RXT_observe(hook.on_enable) { _dispatch.on_enable(); };
        RXT_observe(hook.on_disable) { _dispatch.on_disable(); };
        _map.emplace(k, hook);
    }
